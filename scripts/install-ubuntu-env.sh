...
echo "       sudo git clone <repo-url> /opt/blog-bootstrap"
...
echo "       sudo env APP_NAME=${APP_NAME} SERVICE_USER=${SERVICE_USER} ARTIFACT_TARBALL=/tmp/<artifact>.tar.gz bash /opt/blog-bootstrap/scripts/deploy-ubuntu.sh"
echo "    6. Configure Nginx for public access:"
echo "       sudo env DOMAIN=example.com SERVER_ALIASES=www.example.com APP_PORT=3000 bash /opt/blog-bootstrap/scripts/configure-ubuntu-site.sh"
echo "    7. Optionally request HTTPS once DNS points to this server:"
echo "       sudo env DOMAIN=example.com SERVER_ALIASES=www.example.com ENABLE_HTTPS=1 CERTBOT_EMAIL=admin@example.com bash /opt/blog-bootstrap/scripts/configure-ubuntu-site.sh"
echo
if [[ "${INSTALL_UFW}" == "1" ]]; then
