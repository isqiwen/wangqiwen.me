...
**Artboards** have become the de-facto standard <Citation refId="artboards" label="[2]" /> to convey how a certain UI is supposed to be rendered for
...
Another way in which discovery occurs, usually more painstakingly, is in the _implementation_ process itself.

In the case of video streaming, something that can be easily overlooked (which falls under the category of “errors”) is that the download of the video can stall completely. When this happens, the MediaElement API emits a _stalled_ event.
...
_Lorem Ipsum_ has had a good run. Let’s replace it with data.

<Bibliography>
  <BibliographyItem
    id="programming-models"
    label="[1]"
    title="Evaluating programming models"
  >
    ...
    will inform you about how well the code will fare over time as the product
    matures and the team grows, but the former won’t. It will also empower you
    to foresee what the evolutionary path of the technology looks like.
  </BibliographyItem>
  <BibliographyItem
    id="artboards"
    label="[2]"
    title="Artboards in design tools"
    links={[
      {
        label: "Illustrator",
        href: "https://helpx.adobe.com/illustrator/how-to/work-with-artboards.html",
      },
      { label: "Sketch", href: "https://www.sketchapp.com/" },
    ]}
  >
    Artboards were initially introduced by Illustrator as a way of grouping
    related designs together. Sketch popularized their use to represent designs
    targeting varying devices and screen sizes, and Photoshop later incorporated
    the feature.
  </BibliographyItem>
</Bibliography>
