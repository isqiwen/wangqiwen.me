...
- **Client-side JS and APIs** that get executed later, once the static markup and code is downloaded and executed, allow for effectively infinite dynamism.
  ...
  Pre-computing all pages is not always possible <Citation refId="critical-pages" label="[2]" />, nor desirable. For example, when dealing with data that is _not_ shared between all users and we wouldn't want to cache at the edge <Citation refId="static-security" label="[3]" />.
...
This wonderful effect is particularly pronounced for databases. It's one thing to upgrade the infrastructure of _stateless_ code execution, but dealing with data is a whole new challenge that I'm glad we no longer have to deal with <Citation refId="cosmos-db" label="[4]" />.
...
It has nothing to do with performance or JS and HTML, and everything to do with being altogether unable to deliver on the standards of what a _real_ app can do on the platform^<Citation refId="ios-viewport" label="[5]" />.
...
AMP, although controversial, is a _systematic_, rather than _suggested_ answer to the performance problem. It's very hard, maybe impossible, to create a slow AMP experience, due to the smart constraints and built-in well-optimized components <Citation refId="platform-stdlib" label="[6]" />.
...
For one: the less code you write, the easier to maintain, and [the less likely](#testing-the-jamstack) that [it will be wrong](#all-code-is-wrong). Next.js is a clear example of this. Our data-fetching <Citation refId="swr-ergonomics" label="[7]" /> library [SWR](https://swr.now.sh/) is another. [Zero-config deployments](https://vercel.com/blog/zero-config), another.
...
<Bibliography>
  <BibliographyItem
    id="zero-downtime-rollouts"
    label="[1]"
    title="Zero-downtime rollout tradeoffs"
  >
    Servers can be hard to roll out without downtime: a KubeCon keynote starts
    with the admission,{" "}
    <b>
      &quot;Noticing your customers receive 503&apos;s every now-and-then?&quot;
    </b>{" "}
    By not needing to rotate pods, shut down containers, handle signals, wait
    for grace periods, or configure and execute liveliness probes, static is
    also <b>faster and safer to roll</b>.
  </BibliographyItem>
  <BibliographyItem
    id="critical-pages"
    label="[2]"
    title="Precomputing critical pages"
  >
    When the set of pages to pre-compute is too large and would make build times
    prohibitive, it&apos;s still probably a good idea to pre-compute your most
    critical public pages and do the rest <em>asynchronously</em>.
  </BibliographyItem>
  <BibliographyItem
    id="static-security"
    label="[3]"
    title="Static delivery and security"
  >
    Websites and apps that serve the same static markup and code to all users
    have a drastically simpler security model, which means static is also{" "}
    <b>more secure</b>.
  </BibliographyItem>
  <BibliographyItem id="cosmos-db" label="[4]" title="Cosmos DB">
  ...
  <BibliographyItem id="ios-viewport" label="[5]" title="iOS viewport behavior">
    A mobile web app on iOS Safari cannot be native like an app because the
    viewport size is dynamic and shifts as you scroll to reveal different
    toolbars.
  </BibliographyItem>
  <BibliographyItem
    id="platform-stdlib"
    label="[6]"
    title="Platform standard libraries"
  >
    Rather than native code generation, native mobile apps may owe their
    generally better performance to a rich standard library of well-optimized UI
    components. Go, when compared to Node.js+npm, has similarly demonstrated the
    success of a great standard library for common, performance-critical needs.
  </BibliographyItem>
  <BibliographyItem
    id="swr-ergonomics"
    label="[7]"
    title="SWR and Redux"
    links={[{ label: "SWR", href: "https://swr.now.sh/#basic-data-loading" }]}
  >
    What you do in one line of SWR, you tend to do in a few dozen lines of
    Redux.
  </BibliographyItem>
</Bibliography>
